...
namespace Tonic {
  ...
  public:
    ...
    */
	...
	//! Copy one channel from another TonicFrames
	/*!
	  The \c src and \c dst indices must be between 0 and channels() - 1, and
	  should not be the same number. No range checking is performed (yet)
	*/
	void copyChannel(TonicFrames & src_frames, unsigned int src_channel, unsigned int dst_channel);
		
	//! Copy float array to another specific channel
	/*!
	The \c src and \c dst indices must be between 0 and channels() - 1, and
	should not be the same number. No range checking is performed (yet)
	*/
	void copyToChannel(TonicFloat *src, size_t nFrames, unsigned int dst_channel);
  ...
  inline void TonicFrames::copyChannel(TonicFrames & src_frames, unsigned int src_channel, unsigned int dst_channel)
  {
  #if defined(TONIC_DEBUG)
	  if (src_frames.size() != size())
		  std::ostringstream error;
	  error << "TonicFrames::copyChannel: frame mismatch! (" << src_frames.size() << " != " << size() << ")";
	  Tonic::error(error.str(), true);
  }
  #endif
  TonicFloat *sptr = src_frames.data_ + src_channel;
  TonicFloat *dptr = data_ + dst_channel;
  unsigned int stride = nChannels_;
  ...
  inline void TonicFrames::copyToChannel(TonicFloat *src, size_t nFrames, unsigned int dst_channel)
  {
  #if defined(TONIC_DEBUG)
	  if (src_frames.size() != size())
		  std::ostringstream error;
  ...
  vcopy(dptr, stride, sptr, 1, (unsigned int)nFrames_);
